/**
 * Copyright (c) 2000-present Liferay, Inc. All rights reserved.
 *
 * This library is free software; you can redistribute it and/or modify it under
 * the terms of the GNU Lesser General Public License as published by the Free
 * Software Foundation; either version 2.1 of the License, or (at your option)
 * any later version.
 *
 * This library is distributed in the hope that it will be useful, but WITHOUT
 * ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS
 * FOR A PARTICULAR PURPOSE. See the GNU Lesser General Public License for more
 * details.
 */

package com.rcs.dbservice.model.impl;

import com.liferay.portal.kernel.util.StringBundler;
import com.liferay.portal.model.CacheModel;

import com.rcs.dbService.model.WebformRow;

import java.io.Externalizable;
import java.io.IOException;
import java.io.ObjectInput;
import java.io.ObjectOutput;

import java.util.Date;

/**
 * The cache model class for representing WebformRow in entity cache.
 *
 * @author rikad.ramadhan@rotterdam-cs.com
 * @see WebformRow
 * @generated
 */
public class WebformRowCacheModel implements CacheModel<WebformRow>,
	Externalizable {
	@Override
	public String toString() {
		StringBundler sb = new StringBundler(7);

		sb.append("{webformRowId=");
		sb.append(webformRowId);
		sb.append(", webformTableId=");
		sb.append(webformTableId);
		sb.append(", updateDate=");
		sb.append(updateDate);
		sb.append("}");

		return sb.toString();
	}

	@Override
	public WebformRow toEntityModel() {
		WebformRowImpl webformRowImpl = new WebformRowImpl();

		webformRowImpl.setWebformRowId(webformRowId);
		webformRowImpl.setWebformTableId(webformTableId);

		if (updateDate == Long.MIN_VALUE) {
			webformRowImpl.setUpdateDate(null);
		}
		else {
			webformRowImpl.setUpdateDate(new Date(updateDate));
		}

		webformRowImpl.resetOriginalValues();

		return webformRowImpl;
	}

	@Override
	public void readExternal(ObjectInput objectInput) throws IOException {
		webformRowId = objectInput.readLong();
		webformTableId = objectInput.readLong();
		updateDate = objectInput.readLong();
	}

	@Override
	public void writeExternal(ObjectOutput objectOutput)
		throws IOException {
		objectOutput.writeLong(webformRowId);
		objectOutput.writeLong(webformTableId);
		objectOutput.writeLong(updateDate);
	}

	public long webformRowId;
	public long webformTableId;
	public long updateDate;
}